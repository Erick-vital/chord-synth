...
## Presets y configuraciones de fábrica

Los presets incorporan recetas armónicas por grado como configuraciones de fábrica:

| Preset / Configuración | Formas I–vii° | Estilo | Bajo | Leading |
|---|---|---|---|---|
| Init / Diatónica | tríada en todos los grados | compacto | ninguno | manual |
| Séptimas | séptima en todos los grados | compacto | ninguno | nearest |
| Lo‑Fi Warm | 9, 9, 7, 9, 13, 9, 7 | abierto | raíz | nearest |
| Jazz Tension | 6/9, 11, 9, 9, 13, 11, 7 | rootless | raíz | nearest |

Las configuraciones son datos por grado y no codifican tonos de C. El resolvedor las adapta a la tonalidad y a Mayor o Menor natural.

## Transformaciones temporales

Los colores de acorde son transformaciones no persistentes sobre la especificación guardada del grado activo. Al pulsar un color, el controlador calcula la transformación desde la base guardada, emite únicamente el delta de note-off/note-on y conserva el estado temporal. Al soltarlo, restaura la base usando otro delta. **Fijar en grado** persiste la transformación en el grado activo sin rearticular el acorde que ya está sonando.
...
No se acumulan transformaciones: cambiar de ranura vuelve a partir de la base. Cambiar tonalidad o escala, perder el foco o cerrar el editor libera el acorde activo y termina la transformación.
...
`HarmonyState` es versión 3 y persiste forma, inversión, estilo, octava, regla de calidad, política de quinta, modo/slash de bajo y voice leading para cada uno de los 7 grados. Carga estados v1 y v2 migrando de forma segura la escena seleccionada histórica a la configuración única y descartando escenas no seleccionadas.

Los presets JSON usan `schema_version: 5` conteniendo un único array `harmony.degrees`. Los esquemas 1 a 4 siguen cargando mediante migración retrocompatible; las nuevas escrituras emiten esquema 5. Cargar un preset desde la interfaz siempre libera primero el acorde activo y sincroniza APVTS, HarmonyState, controlador, toolbar, pads y diseñador.
