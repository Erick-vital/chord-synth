...
void initLegacySceneDefaults(music::HarmonyConfiguration& config, int sceneIndex) noexcept {
    const auto defaultSpec = getLegacyDefaultSpec(sceneIndex);
    for (int degIdx = 0; degIdx < 7; ++degIdx) {
        config.setSpec(degIdx, defaultSpec);
    }
}
...
juce::ValueTree HarmonyState::toValueTree() const {
    ...
    for (int degIdx = 0; degIdx < 7; ++degIdx) {
        const auto spec = configuration.getSpec(degIdx);
        ...
        degreesNode.appendChild(degNode, nullptr);
    }
...
bool HarmonyState::loadFromValueTree(const juce::ValueTree& vt) noexcept {
    ...
    if (version == 3) {
        auto degreesNode = vt.getChildWithName("Degrees");
        if (degreesNode.isValid()) {
            for (const auto& degNode : degreesNode) {
                ...
                configuration.setSpec(degIdx, spec);
            }
        }
        return true;
    }

    // Migration for version 1 and 2
    int legacySelectedScene = 0;
    if (vt.hasProperty("selectedScene")) {
        const int s = vt.getProperty("selectedScene");
        legacySelectedScene = std::clamp(s, 0, 3);
    }

    if (version == 1) {
        initLegacySceneDefaults(configuration, legacySelectedScene);
    }
    ...
    if (scenesNode.isValid()) {
        for (const auto& sceneNode : scenesNode) {
            ...
            if (sceneIdx != legacySelectedScene)
                continue;
